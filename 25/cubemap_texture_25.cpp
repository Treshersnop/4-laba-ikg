...
{
    glGenTextures(1, &m_textureObj);//генерации объекта текстуры
    glBindTexture(GL_TEXTURE_CUBE_MAP, m_textureObj);//привязывается к специальной метке GL_TEXTURE_CUBE_MAP

    Magick::Image* pImage = NULL;
    Magick::Blob blob;
    //загрузка изобрадений
    for (unsigned int i = 0; i < ARRAY_SIZE_IN_ELEMENTS(types); i++) //стороны кубической текстуры
    {
